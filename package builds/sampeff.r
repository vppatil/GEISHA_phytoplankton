#' Visually assess change in sampling effort over time (From Dietmar Straile)
#'
...
#' 
#' sampeff(lakegeneva,column=11) #column 11 contains biovolume
...
sampeff = function(b_data,column,save.pdf=F,lakename='',datecolumn='date_dd_mm_yy',dateformat='%d-%m-%y') {
  ...
  sbio = aggregate(formula(paste('abundance ~',datecolumn)) , data = b_data, sum)
  names(sbio)[2] = 'sabundance'
  sbio2 = merge(b_data, sbio, by = datecolumn, all = T)
  sbio2$relabundance = sbio2$abundance /sbio2$sabundance
  minbio = aggregate(formula(paste('relabundance ~',datecolumn)), data = sbio2, min)

  if(save.pdf){pdf(paste('sampeff',lakename,'.pdf',sep=''))}
  
  plot(I(100 * relabundance) ~ date_dd_mm_yy, data = minbio, log = 'y', xlab = '', ylab = 'Minimum contribution [%]')
