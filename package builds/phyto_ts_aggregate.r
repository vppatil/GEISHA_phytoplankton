                            ...
                            remove.rare=F,fun=sum,format='%d-%m-%y')
{
  ...
  groupingvars=na.omit(groupingvars)
  ...
  agg.func=as.formula(paste(AbundanceVar,"~ date_dd_mm_yy + ",groupingvars))
  
  ts.agg=aggregate(formula=agg.func,data=phyto.data,FUN=fun)
