...
#'of being sampled at any point in the time series.
#'
...
#' 
#' accum(lakegeneva,column=NA,n=1000,save.pdf=F)

accum = function(b_data, phyto_name='phyto_name',column, n=50,save.pdf=F,lakename='') {
  ...
  if(save.pdf)
  {
    pdf(paste('phytoSAC',lakename,'.pdf',sep=''))
    par(mfcol=c(2,1))
    par(mar=c(3,4,1,1))
  }
